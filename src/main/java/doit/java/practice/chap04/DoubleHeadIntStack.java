...
public class DoubleHeadIntStack {
    ...
    private int capacity;   // 스택 용량 (A+B)
    ...
    // push
    // 스택에 x를 푸시
    public int push(AorB sw, int x) throws DoubleHeadIntStack.OverflowDoubleHeadIntStackException {
        if (ptrA >= capacity)
            throw new DoubleHeadIntStack.OverflowDoubleHeadIntStackException();
        switch (sw) {
    ...
    public int pop(AorB sw) throws DoubleHeadIntStack.EmptyDoubleHeadIntStackException {
        int x = 0;
        switch (sw) {
            case StackA:
                if (ptrA <= 0)
                    throw new DoubleHeadIntStack.EmptyDoubleHeadIntStackException();
                x = stk[--ptrA];
                break;
            case StackB:
                if (ptrB >= capacity - 1)
    ...
    public int peek(AorB sw) throws DoubleHeadIntStack.EmptyDoubleHeadIntStackException {
        int x = 0;
        switch (sw) {
            case StackA:
                if (ptrA <= 0)
